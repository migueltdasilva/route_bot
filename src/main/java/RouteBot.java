...
public class RouteBot extends Bot {
    ...
    private static final String[] vTrips =
            new String[] {"Хочу в Тундру на вертолете! 18-20 декабря"};
    ...
    static {
        vQuestions[0] = new String[]{
                ...
                "Какой был самый классный/странный/ПУГАЮЩИЙ транспорт, на котором ты (так или иначе) передвигался?",

                " Какой коктейль легко сделать в полярных условиях?",
                ...
                "Видел ли ты когда-нибудь северное сияние?" +
                    " Как ты думаешь, если бы оно сопровождалось звуком," +
                    " то что за звук это бы был?" +
                    " Запиши аудиосообщение с этим звуком. \n" +
                    "(Можно просто изобразить гром, если больше ничего не придумывается, ор в микрофон мы всегда приветствуем)"};
        ...
        if (cmd == Command.START) {
            ...
            if (vTrips.length == 0) {
                sendMsgNoKeyboard(
                        ...
                        "Привет! Скоро тут можно будет записаться в поездку рута, а пока новых поездок нет." +
                                " Следи за обновлениеями в группе на фейсбуке и возвращайся ко мне, как что-то увидишь там!️");
            } else {
                sendMsg(
